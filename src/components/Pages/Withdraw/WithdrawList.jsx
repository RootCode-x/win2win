...
export default function WithdrawList({ session }) {  
  ...
  useEffect(() => {
    const fetchDepositReport = async () => {
      setIsLoading(true);
      try {
        const response = await axios.get(`${api_path}/withdraw/withdraw?report=true&user_id=${session.user.user_id}`, {
                                        ...
                                        <table className="w-100 text-center p2-bg">
                                            ...
                                            witdrawReport.map((deposit, index) => (
                                                    <tr key={index}>
                                                        ...
                                                        <td className="text-nowrap">{deposit.withdraw_amount}</td> 
                                                        {/* <td className="text-nowrap">{deposit.chargedAmount}</td> 
                                                        <td className="text-nowrap">{deposit.withdraw_amount_in_bdt}</td>  */}
