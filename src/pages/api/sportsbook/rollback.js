...
function calculateXSign(headers, params) {
  ...
  async function callback(req, res) {
    ...
    if (action === 'rollback') {
      ...
      try {
        const { amount, bet_transaction_id, betslip_id, currency, parent_transaction_id, player_id, transaction_id, session_id, sportsbook_uuid, type } = req.body;
        ...
        const originalTransaction = await Sportsbooktb.findOne({
          $and: [
        ...
        if (!originalTransaction) {
          return res.status(200).json({
            error_code: "INTERNAL_ERROR",
            error_description: "Original transaction not found.",
          });
        }
